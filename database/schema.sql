...
-- rule. D043 later added four derived-data tables without changing this
-- resource-removal requirement.
--
...
-- Tables: bookmarks, helpful_marks, ai_source_versions,
--         ai_processing_states, ai_chunks, ai_embeddings, ai_outputs
-- Depends on: accounts, resources (Parts 1–2)
-- Target: MySQL/MariaDB via XAMPP/phpMyAdmin
-- Engine: InnoDB (required for foreign keys)
-- Charset: utf8mb4
...
-- ---------------------------------------------------------------------
-- Table: ai_source_versions
-- Purpose: immutable source identity and extracted text for one resource
-- version. Exactly one current version is permitted per resource.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Table: ai_processing_states
-- Purpose: fail-closed capability readiness and dependency identity for
-- one source version. This is state, not a provider request log.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Table: ai_chunks
-- Purpose: locator-preserving segments for one verified source version.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Table: ai_embeddings
-- Purpose: auditable embedding vectors bound to a chunk and measured
-- candidate configuration. JSON is retained for this bounded MVP.
-- ---------------------------------------------------------------------
...
CREATE TABLE ai_outputs (
    ...
    -- Reproducibility identity for active AI output. Invalidated legacy
    -- rows may keep these fields NULL, but draft/retained rows may not.
    candidate_configuration_id  VARCHAR(100) NULL,
    prompt_template_version     VARCHAR(100) NULL,
    ...
    KEY idx_ai_outputs_source_version (source_version_id),

    CONSTRAINT fk_ai_outputs_resource
        FOREIGN KEY (resource_id) REFERENCES resources (id)
        ON DELETE RESTRICT,

    CONSTRAINT fk_ai_outputs_source_resource
