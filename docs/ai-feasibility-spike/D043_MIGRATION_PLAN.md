...
## 9. Completed Guarded Semantic-Retrieval Backend

The separately approved backend checkpoint adds `SemanticRetrievalRepository` and `GuardedSemanticRetrieval` without changing the 22-table schema. The service is default-off through `AI_SEMANTIC_RETRIEVAL_ENABLED=false`, also requires the live `ai_enabled` gate, embeds one bounded query at request time, and persists no query vector or retrieval history.

Candidates must remain Approved, available, current, source-file-matched, and ready for extraction, segmentation, and the exact accepted embedding configuration/model/digest/dimension. Existing course, subject, year-level, resource-type, and tag filters are preserved. Each result is revalidated immediately before return, repeated chunks collapse to one best passage per resource, and cosine remains an internal ranking value rather than an evidence-sufficiency threshold. Disabled, unavailable, missing-file, or malformed semantic behavior returns ordinary Approved-only metadata search.

`tests/ai/run_d043_semantic_retrieval.php` passed 43/43 disposable checks, including late requester-disable rejection, with deterministic fake vectors, zero real provider/model requests, zero live database writes, zero query-vector persistence, and automatic cleanup of its random database/storage. It added no public route/UI, related-resource behavior, generation, commit, or push. Any user-facing route/browser acceptance remains a separate gate.
