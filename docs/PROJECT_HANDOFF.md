...
4. **Completed: use the accepted final recommendation to approve and propagate D043.**
   ...
   * Gate 5C passed 18/18 live related-resource checks using two controlled Security resources and two controlled Usability resources under one subject. A shared-active-tag metadata fallback returned the expected peer for 4/4 targets, excluded cross-topic same-subject results, and failed closed for ineligible resources, files, tags, and requesters. Protected links used `/resources/{id}` and resolved through Approved-only lookup. The later default-off detail-page integration reused this exact seam and passed 28/28 focused surface checks with no model call, embedding rerun, or database write. A separate 44/44 read-only broader-corpus reconciliation covered 25 accepted resources in five reviewed relation groups and reproduced 100% precision, 72% recall, 83.72% F1, useful displayed peers for 19/25 resources, and six false safe-no-result resources. It supports shared tags only as a conservative incomplete fallback and does not select a final relation rule.
...
7. **Completed: add one guarded local processing path.**
...
8. **Completed through owner browser acceptance of the default-off public semantic-search surface.**
   ...
   * `tests/ai/run_d043_semantic_retrieval.php` remains 43/43 on a disposable database, including late requester-disable rejection, with zero real model requests and zero live database writes. The added `tests/ai/run_d043_semantic_search_surface.php` passed 23/23 focused checks for the explicit search-method choice, safe fallback, escaped matched context, complete-word excerpt endings, raw-score suppression, and default-off configuration.
   ...
   * One-time owner browser acceptance confirmed standard search as the default, the expected heuristic-evaluation result and locator, Security-tag filtering to Resources 35 and 36, empty-query metadata fallback, a 390-pixel responsive layout without horizontal overflow, complete-word excerpt endings, and clear fallback after restoration. The environment hash, database gate state, and all five AI-table counts matched the pre-activation baseline after both gates were restored off.
   * The Resource 36-over-35 top-one miss remains preserved. The public search route exists but remains default-off; no no-result/evidence-sufficiency threshold, automatic activation, related-resource application integration, final candidate, or generation capability was introduced.
...
The bounded related-resource evaluation, safe no-result controls, source-attribution presentation, follow-up comparison, lifecycle/fallback validation, external grounded comparison, summary/suggestion evaluation, final evidence reconciliation, D043 live/canonical migration verification, provider-neutral persistence, guarded local resource processing, guarded semantic retrieval, and owner-browser-accepted default-off public semantic-search surface are complete. The related-resource detail surface is also implemented, offline-verified, and browser-accepted in both feature-off and temporarily enabled states at desktop and 390-by-844 mobile viewports. The temporary activation displayed the expected Security and Usability peer links, preserved protected Approved-only routing, and was followed by exact environment, database-setting, and 22-table-count restoration. A later 25-resource offline reconciliation found the shared-tag fallback precise but incomplete: 100% precision, 72% recall, and six false safe-no-result resources. Because the live repository contains only five Approved and available resources, this is not a representative live quality score. The reconciled outcome remains **Partially feasible — alternative or mixed architecture required**. No final relation rule, generation provider/model, or generated inquiry is selected. Broader application fallback verification remains separate.
