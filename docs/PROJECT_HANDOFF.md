...
| `AI_FEASIBILITY_SPIKE.md` | Complete and accepted as the controlling feasibility-spike specification, Sections 1–26. The spike is partially executed through retrieval, bounded local generation, deterministic grounded-response and session/lifecycle controls, bounded related-resource evaluations, isolated source-attribution presentation, the two-model natural-language follow-up comparison, Gate 5A/5B safety/lifecycle validation, the 18-case Gate 5C live relation-metadata/link validation, and the Gate 5D Groq/GPT-OSS candidate review plus one harmless synthetic connectivity probe. Neither local follow-up candidate met all accepted quality and latency criteria or is selected. The external candidate returned one valid synthetic response but received no BPC evidence and is not registered or selected. Persistent derived-data cleanup, complete application fallback, production-session follow-up, processing-readiness/retrieval integration, any approved external grounded comparison, and the final recommendation remain pending. No final provider, model, relation rule, retrieval layer, citation UI, database upgrade, second database, or schema expansion has been selected. |
...
| `PROJECT_HANDOFF.md` | This updated version. It records the accepted specification, verified MariaDB 10.4.32 schema baseline, completed retrieval, synthetic preflight, bounded local grounded-generation, deterministic grounded-response/session/lifecycle controls, bounded related-resource evaluation, safe no-result and positive regressions, isolated source-attribution presentation, natural-language follow-up comparison, Gate 5A/5B safety validation, Gate 5C live relation-metadata/link validation, and the remaining feasibility work without selecting a final AI/retrieval architecture. |
...
`AI_FEASIBILITY_SPIKE.md` is complete and accepted. The deterministic grounded-response control layer passed 21/21 cases, the model-independent session/lifecycle checkpoint passed 200/200 fixed cases, the bounded related-resource evaluation met its two scored positive-case thresholds, the separate safe no-result related-resource boundary control passed its one predeclared synthetic case, the metadata-guarded positive regression passed its five cases at 100% expected-resource display and 100% reviewed top-three usefulness, and isolated source-attribution presentation passed 10/10 cases. The registered natural-language follow-up comparison completed 20/20 local requests, but neither model met the full accepted criteria: Llama preserved 10/10 context references but only 8/10 grounded correct answers with one critical RBAC error, while Qwen produced 8/10 grounded correct answers and only 8/10 correct context interpretations. Both missed the 15-second median target. Gate 5A passed 18/18 deterministic controls, Gate 5B passed 19/19 rollback-based lifecycle/fallback checks, and Gate 5C passed 18/18 live relation-metadata/link checks using four controlled Approved synthetic resources. Gate 5C achieved 4/4 expected-peer top-five coverage and 4/4 reviewed top-three usefulness while excluding self, same-subject cross-topic, and currently ineligible results. It remains an unrouted small-corpus metadata-fallback proof, not a final relation-rule selection. Gate 5D then passed 151/151 offline provider, fixture-authorization, credential-location, and synthetic-request-contract guards for GroqCloud `openai/gpt-oss-120b`. Its separately approved one-request synthetic probe returned HTTP 200 and the exact strict response in 1,668.951 ms with 217 total tokens, zero retries, and zero BPC evidence transmitted. Persistent derived-data cleanup, production-session follow-up, processing-readiness/retrieval integration, complete application fallback, approved external quality evidence, and final evidence reconciliation remain required before application integration or a final architecture decision.
...
4. **Continue Gate 5 live safety and feasibility validation.**
   ...
   * Gate 5D passed 151/151 offline checks for the reviewed GroqCloud `openai/gpt-oss-120b` candidate. After project-specific key, Zero Data Retention, model-allowlist, quota, and explicit-approval controls were confirmed, one harmless synthetic request returned HTTP 200 and the exact strict response in 1,668.951 ms using 217 total tokens. It sent no BPC content, retried zero times, persisted neither response nor key, and left the accepted payload-manifest register empty.
   * Gates 5A/5B/5C and Gate 5D prove bounded unrouted control and preparation/connectivity seams only. They add no AI route/UI, persist no accepted-run test mutation, change no schema, and select no final relation rule, model, provider, or architecture.
   * Next, decide whether an externally generated grounded comparison is justified. Any such comparison requires a reviewed payload manifest, exact selected synthetic evidence, a declared request/token/cost ceiling, and separate explicit approval.

Remaining order:

5. **Continue the accepted feasibility spike.**

   * Completed checkpoints cover extraction success/failure, source-location fidelity, corrected segmentation, complete local embedding, PHP cosine correctness/timing, standalone retrieval relevance/latency, targeted manual relevance review, bounded local grounded-generation comparison, deterministic grounded-response/session/lifecycle controls, bounded related-resource evaluation, metadata-guarded safe no-result and positive regression, isolated end-user source-attribution presentation, natural-language follow-up comparison, rollback-based live lifecycle/fallback validation, and live relation-metadata/link validation.
   ...
   * The Gate 5A/5B/5C candidate foundation now covers default-off configuration, active-account and Approved/available source checks, exact source-file reference and protected-file checks, rollback-tested lifecycle/stale/missing-file transitions, second-point revalidation, protected `/resources/{id}` attribution shaping, bounded session-only identifiers, metadata-search/download fallback, and shared-active-tag live relation fallback with same-subject cross-topic exclusion.
   * Remaining checkpoints include the separately approved synthetic external connectivity probe, processing-readiness and retrieval integration design, real request classification, production-session natural-language follow-up, persistent derived-data cleanup, complete application fallback, any justified external grounded-generation comparison, generation dependencies, maintainability, and any missing resource observations.
...
- The spike is partially executed through extraction, corrected segmentation, complete local embedding, PHP cosine validation, standalone retrieval, manual relevance review, audited versioned ground-truth evaluation, bounded local grounded-generation comparisons, deterministic grounded-response/session/lifecycle controls, bounded related-resource evaluations, isolated source-attribution presentation, and the two-model natural-language follow-up comparison. Gate 5A passed 18/18 model-independent checks, Gate 5B passed 19/19 rollback-based lifecycle/fallback checks, and Gate 5C passed 18/18 live relation-metadata/link checks on 2026-08-13 with zero accepted-run persistent state change, zero protected-file modification, and zero real provider calls.
...
The bounded related-resource evaluation, separate safe no-result boundary control, metadata-guarded positive regression, isolated source-attribution presentation checkpoint, natural-language follow-up comparison, Gate 5B lifecycle/fallback validation, Gate 5C live relation-metadata/link validation, and Gate 5D external-candidate offline and synthetic-connectivity validation are complete. The unguarded positive-case evaluation met the accepted top-five and human top-three thresholds while preserving four weak or unrelated findings. The metadata-guarded configuration passed one predeclared synthetic safe no-result case with five raw neighbors, zero displayed suggestions, a safe message, and no selected cosine threshold; its five-case positive regression retained expected resources in 5/5 cases and achieved 15/15 clearly or meaningfully related reviewed top-three suggestions without changing the raw evidence. Gate 5C then passed 18/18 checks using two Security and two Usability resources in the live database, with 4/4 expected-peer top-five coverage, 4/4 reviewed top-three usefulness, same-subject cross-topic exclusion, protected links, and rollback-tested lifecycle exclusion. Gate 5D added one provider-connectivity result only: HTTP 200, exact strict output, 1,668.951 ms, and 217 tokens for a harmless project-independent probe. This does not select a final relation rule, generation provider, model, or application UI. The next work is a decision on external grounded comparison, live production-session follow-up, processing-readiness/retrieval integration, persistent derived-data cleanup, complete application fallback validation, and final evidence reconciliation.
