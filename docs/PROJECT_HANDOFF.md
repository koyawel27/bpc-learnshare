...
| `AI_FEASIBILITY_SPIKE.md` | Complete and accepted as the controlling feasibility-spike specification, Sections 1–26. The spike is partially executed through retrieval, bounded local generation, deterministic grounded-response and session/lifecycle controls, bounded related-resource evaluations, isolated source-attribution presentation, the two-model natural-language follow-up comparison, Gate 5A/5B safety/lifecycle validation, and the 18-case Gate 5C live relation-metadata/link validation. Neither follow-up candidate met all accepted quality and latency criteria or is selected. Persistent derived-data cleanup, complete application fallback, production-session follow-up, processing-readiness/retrieval integration, any justified external comparison, and the final recommendation remain pending. No final provider, model, relation rule, retrieval layer, citation UI, database upgrade, second database, or schema expansion has been selected. |
...
4. **Continue Gate 5 live safety and feasibility validation.**
   ...
   * Gate 5A/5B/5C prove bounded unrouted control seams only. They add no AI route/UI, call no real model/provider, persist no accepted-run test mutation, change no schema, and select no final relation rule, model, or architecture.
   * Next, reconcile the remaining final-feasibility evidence and prepare a bounded recommendation identifying which gaps block or defer Gate 6 integration.
...
5. **Continue the accepted feasibility spike.**
   ...
   * Remaining checkpoints include processing-readiness and retrieval integration design, real request classification, production-session natural-language follow-up, persistent derived-data cleanup, complete application fallback, any justified external-generation comparison, generation dependencies, maintainability, and any missing resource observations.
