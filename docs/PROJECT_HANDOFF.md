...
| `AI_FEASIBILITY_SPIKE.md` | Complete and accepted as the controlling feasibility-spike specification, Sections 1–26. The spike is partially executed through retrieval, bounded local generation, deterministic grounded-response and session/lifecycle controls, bounded related-resource evaluations, isolated source-attribution presentation, the two-model natural-language follow-up comparison, Gate 5A/5B safety/lifecycle validation, the 18-case Gate 5C live relation-metadata/link validation, the Gate 5D Groq/GPT-OSS grounded comparison, and Gate 5E summary/controlled-suggestion measurement. Gate 5E preserved one failed v1 provider-schema attempt and then completed an approved v2 run whose summaries, controlled tags, limited metadata, usability, latency, and safety boundaries met every accepted Gate 5E threshold. Neither local follow-up candidate is selected. The external candidate still failed strict grounded-answer claim support and attribution, so its Gate 5E pass does not accept or select it. Persistent derived-data cleanup, complete application fallback, production-session follow-up, processing-readiness/retrieval integration, final evidence reconciliation, and the final recommendation remain pending. No final provider, model, relation rule, retrieval layer, citation UI, database upgrade, second database, or schema expansion has been selected. |
