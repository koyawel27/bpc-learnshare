...
| `AI_FEASIBILITY_SPIKE.md` | Complete and accepted as the controlling feasibility-spike specification, Sections 1–26. The spike is partially executed through retrieval, bounded local generation, the 21-case grounded-response control layer, the registered 200-case deterministic session/lifecycle checkpoint, the registered five-case related-resource evaluation, the registered one-case safe no-result related-resource boundary control, the registered five-case metadata-guarded positive regression, the registered ten-case isolated source-attribution presentation checkpoint, the registered ten-case natural-language follow-up comparison for two local models, and the 19-case Gate 5B live lifecycle/fallback control-seam validation. Neither follow-up candidate met all accepted quality and latency criteria or is selected. Live PHP/database relation-metadata and attribution-link integration, persistent derived-data cleanup, complete application fallback, any justified external comparison, and the final recommendation remain pending. No final provider, model, retrieval layer, citation UI, database upgrade, second database, or schema expansion has been selected. |
...
| `PROJECT_HANDOFF.md` | This updated version. It records the accepted specification, verified MariaDB 10.4.32 schema baseline, completed retrieval, synthetic preflight, bounded local grounded-generation, deterministic grounded-response controls, deterministic session/lifecycle controls, bounded related-resource evaluation, the separate safe no-result boundary control, the metadata-guarded positive regression, isolated source-attribution presentation, natural-language follow-up comparison, Gate 5A/5B control-seam validation, and the remaining feasibility work without selecting a final AI/retrieval architecture. |
...
* **AI/retrieval architecture remains unresolved while the feasibility spike is partially executed.**
  ...
  * The 21-case deterministic grounded-response control layer, 200-case model-independent session/lifecycle control checkpoint, five-case related-resource evaluation, one-case safe no-result related-resource boundary control, five-case metadata-guarded positive regression, ten-case isolated source-attribution presentation checkpoint, ten-case two-model natural-language follow-up comparison, and 19-case Gate 5B live lifecycle/fallback control-seam validation are complete. The follow-up runs preserved their failed quality and latency findings; the guarded regression is bounded to frozen relation groups; live database-backed relation-metadata mapping, attribution links, production-session follow-up, persistent derived-data cleanup, and complete application fallback remain incomplete.
...
4. **Continue Gate 5 live safety and feasibility validation.**
   ...
   * Gate 5A/5B prove their bounded control seam only. They add no AI route/UI, call no real model/provider, persist no test mutation, change no schema, and select no model or architecture.
   * Next, validate live relation-metadata and attribution-link integration, reconcile the remaining final-feasibility evidence, and decide whether Gate 6 integration is justified.

Remaining order:

5. **Continue the accepted feasibility spike.**

   * Completed checkpoints cover extraction success/failure, source-location fidelity, corrected segmentation, complete local embedding, PHP cosine correctness/timing, standalone retrieval relevance/latency, targeted manual relevance review, bounded local grounded-generation comparison, deterministic grounded-response controls, deterministic session/lifecycle controls, bounded related-resource evaluation, metadata-guarded safe no-result and positive regression, isolated end-user source-attribution presentation, natural-language follow-up comparison, and rollback-based live lifecycle/fallback control-seam validation.
   ...
   * The Gate 5A/5B candidate foundation now covers default-off configuration, active-account and Approved/available source checks, exact source-file reference and protected-file checks, rollback-tested Hidden/Restricted/Removed/Replaced and stale/missing-file transitions, second-point revalidation, protected `/resources/{id}` attribution shaping, bounded session-only identifiers, and metadata-search/download fallback checks.
   * Remaining checkpoints include live subject/topic/tag relation mapping, actual attribution-link integration, processing-readiness and retrieval integration design, real request classification, production-session natural-language follow-up, persistent derived-data cleanup, complete application fallback, any justified external-generation comparison, generation dependencies, maintainability, and any missing resource observations.
...
* validate the metadata-guarded related-resource behavior against live subject/topic/tag mapping, eligibility revalidation, and protected resource links before application behavior;
...
The bounded related-resource evaluation, separate safe no-result boundary control, metadata-guarded positive regression, isolated source-attribution presentation checkpoint, natural-language follow-up comparison, and Gate 5B live lifecycle/fallback control-seam validation are complete. The unguarded positive-case evaluation met the accepted top-five and human top-three thresholds while preserving four weak or unrelated findings. The metadata-guarded configuration passed one predeclared synthetic safe no-result case with five raw neighbors, zero displayed suggestions, a safe message, and no selected cosine threshold; its five-case positive regression retained expected resources in 5/5 cases and achieved 15/15 clearly or meaningfully related reviewed top-three suggestions without changing the raw evidence. The attribution checkpoint passed its fixed presentation, source-identity, locator, omission, fail-closed, escaping, and responsive-layout cases without selecting the final application UI. The follow-up comparison completed 20 local requests but preserved failed quality and latency outcomes for both models. The next work is live production-session follow-up, relation-metadata and attribution-link integration, persistent derived-data cleanup, complete application fallback validation, and final evidence reconciliation.
