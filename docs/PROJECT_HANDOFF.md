...
| `AI_FEASIBILITY_SPIKE.md` | Complete and accepted as the controlling feasibility-spike specification, Sections 1–26. The spike is partially executed through retrieval, bounded local generation, deterministic grounded-response and session/lifecycle controls, bounded related-resource evaluations, isolated source-attribution presentation, the two-model natural-language follow-up comparison, Gate 5A/5B safety/lifecycle validation, the 18-case Gate 5C live relation-metadata/link validation, and the Gate 5D Groq/GPT-OSS preparation, connectivity, and guarded six-case grounded comparison. Gate 5E now has an offline eight-resource summary/controlled-suggestion contract prepared and validated at 234/234 checks; its first exact eight-request candidate packet passed 104/104 preparation and 187/187 saved-packet audit checks. No summary or suggestion generation has occurred. Neither local follow-up candidate met all accepted quality and latency criteria or is selected. The registered external candidate met latency and manual-usefulness criteria but failed strict claim-support and source-attribution criteria; it is not accepted, selected, or integrated. Persistent derived-data cleanup, complete application fallback, production-session follow-up, processing-readiness/retrieval integration, summary/suggestion measurement, and the final recommendation remain pending. No final provider, model, relation rule, retrieval layer, citation UI, database upgrade, second database, or schema expansion has been selected. |
...
4. **Continue Gate 5 live safety and feasibility validation.**
   ...
   * Gate 5E has a fixed offline summary/controlled-suggestion contract for eight accepted synthetic extraction inputs (two per readable format), five seed-backed Active demo tags, two non-persistent test-only Inactive tags, and the `subject`/`resource_type`/`topic` metadata subset. Its offline validator passed 234/234 checks. The exact eight-request Groq/GPT-OSS preview passed 104/104 preparation checks and its saved seven-file packet passed 187/187 audit checks. No provider request, credential read, accepted registration, generation result, or candidate selection occurred; live execution remains separately approval-gated.
   * Gates 5A/5B/5C and Gate 5D prove bounded unrouted control and feasibility seams only. They add no AI route/UI, change no schema, and select no final relation rule, model, provider, or architecture.
...
5. **Continue the accepted feasibility spike.**
   ...
   * Remaining checkpoints include Gate 5E human payload review and later separately approved summary/suggestion execution and measurement, processing-readiness and retrieval integration design, real request classification, production-session natural-language follow-up, persistent derived-data cleanup, complete application fallback, provider interruption/dependency handling, generation maintainability, and any missing resource observations.
