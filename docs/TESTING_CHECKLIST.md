...
The original Gate 5C run did not add a route or UI, rerun retrieval or embeddings, call a model, select a final relation rule, or authorize Gate 6. The later default-off resource-detail integration reused the accepted class and passed `tests/ai/run_gate5c_related_resources_surface.php` at 28/28 with zero model requests, embeddings, live writes, feature activation, schema/register changes, commits, or pushes. Owner browser acceptance of the feature-off fallback then passed on Resource 35 at desktop and 390-by-844 mobile viewports with the download and repository-search paths preserved, zero suggestion links, no protected filename exposure, and no horizontal overflow. During the approved one-time activation, Resource 35 displayed only the expected protected link to Resource 36 and Resource 37 displayed only the expected protected link to Resource 38; both were labelled Approved, self-results and internal reason/hash fields were absent, and the enabled mobile surface had no horizontal overflow. The original environment SHA-256, absent `ai_enabled` database row, and all 22 table counts were restored exactly afterward. Gate 5C's 100% quality figures still apply only to the four controlled live resources.

`tests/ai/run_gate5c_broader_related_resource_reconciliation.php` later passed 44/44 read-only checks against the accepted 25-resource, five-group corpus. Independent recomputation across 600 ordered non-self resource pairs reproduced 72 true positives, 0 false positives, 28 false negatives, and 500 true negatives: 100% precision, 72% recall, and 83.72% F1. Nineteen of 25 resources received at least one useful displayed peer; six returned a safe no-result despite having a reviewed useful peer. These figures measure shared-tag candidate inclusion, not ranking or end-user usefulness. The live inventory contained only five Approved and available resources, so no broader live score was claimed. No model/provider request, embedding or retrieval rerun, database write, threshold/evidence/register/schema change, candidate decision, commit, or push occurred.
...
| INT-AI-011 | P1 | Open an Approved resource detail with Related Resources disabled or enabled | Clear repository fallback or a bounded eligible Approved-only set | Passed for bounded integration — 28/28 surface checks, 18/18 live backend regression, feature-off fallback, temporarily enabled expected-pair links, mobile layout, and exact restoration passed; 44/44 offline broader-corpus reconciliation confirms the fallback is precise but incomplete |
