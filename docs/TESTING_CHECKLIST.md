...
| AI-REL-001 | P1 | Request related resources | Small relevant Approved-only set with live filters | Passed for bounded integration — 18/18 live Gate 5C backend, 28/28 default-off surface checks, and feature-off/temporarily-enabled desktop/mobile owner-browser acceptance passed; broader corpus quality remains separate |
...
| INT-AI-011 | P1 | Open an Approved resource detail with Related Resources disabled or enabled | Clear repository fallback or a bounded eligible Approved-only set | Passed for bounded integration — 28/28 surface checks, 18/18 live backend regression, feature-off fallback, temporarily enabled expected-pair links, mobile layout, and exact restoration passed |
